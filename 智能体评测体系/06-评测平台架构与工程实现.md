...
| **DeepEval** | RAG 类指标实现（有据性、幻觉、上下文相关性） | 指标实现成熟，省掉从零写 Judge 提示词 | 指标口径要对照 [`01` 分册](01-指标体系与指标字典.md)校对，别直接采信默认口径 |
| **AgentEvals** | 轨迹匹配（B-07 工具序列） | 现成的 trajectory match 实现 | 默认的严格匹配模式不要用，改成节点/边 F1 |
| **Langfuse** 或 **Arize Phoenix** | 证据层存储 + 轨迹查看器 + 数据集管理 | OTel 原生、可自托管、自带转录本查看 UI | 二选一即可，别都上。看板做在哪一边要一次定死 |
| **Giskard** | 安全与鲁棒性对抗样本生成 | 能按自然语言描述自动生成对抗测试 | 生成的样本要人工筛，误报率不低 |
| **OpenTelemetry GenAI 语义约定** | 埋点字段标准 | 生态兼容，换可观测平台不用重新埋点 | — |
