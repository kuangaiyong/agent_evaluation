...
### 7.1 一个可对照的工业实例：微软怎么拆 Sales Agent

微软给 Microsoft 365 Copilot 的销售智能体做评测时，走的就是"按能力拆"这条路——把一个产品拆成 **Sales Research Bench**（研究能力）、**Sales Qualification Bench**（线索资格判定）、**Sales Agent Bench**（端到端销售工作流）三个子基准，各有各的任务集与判据。几个细节值得抄：

**① 权重和是 100%，且权重差距拉得很开。** Sales Research Bench 的 8 个维度：

| 维度 | 权重 | | 维度 | 权重 |
|---|---|---|---|---|
| Text Groundedness | 25% | | Schema Accuracy | 10% |
| Chart Groundedness | 25% | | Chart Relevance | 5% |
| Text Relevance | 13% | | Chart Fit | 5% |
| Explainability | 12% | | Chart Clarity | 5% |

两个"有据性"维度合计占一半——**权重要压在"会不会编"上，而不是平均分给每个维度**。这与本体系 rubric 非 gate 项权重和为 1.0 的硬规则是同一套做法（见 [`02` 分册](02-评测集构建与治理规范.md)）。

**② 环境里要故意埋坑。** 他们的测试 schema 刻意包含自定义表与标准表的**重名列**，用来测智能体在真实企业数据混乱度下的表现。对应到你们，就是评测用的数据库不能是干净的样板库，得带上历史遗留的脏字段。

**③ 对比时要保证数据对等。** 与 ChatGPT / Claude 对比时，通过 MCP SQL connector 让对方访问**镜像的同一份数据**，理由写得很直白——要隔离的是推理与编排能力的差异，不是数据可达性的差异。这条在你们做"自研 vs 采购"选型时直接可用：给两边同样的检索结果，才知道差距是不是出在模型上。

**④ 打分区间设了地板。** 各维度打 **20（最差）到 100（最好）**，不是 0–100。设地板的好处是避免个别维度归零把加权总分拖成噪声，代价是分数不再线性可比——**要么学它设地板并在报告里说明，要么就用 0–1，别混着来**。

**⑤ 他们没做的那件事，你们要做。** 公开报告里没有披露任何判官与人工的一致性统计，所以"78.2 分 vs 54.1 分"这 24 分差距里有多少是判官口径偏移，读者无从判断。本体系把 L-01/L-02 设成硬门槛就是为了堵这个口子，详见 [`03` 分册 §5.2](03-评分器设计与LLM-Judge校准规范.md#52-参考基准)。

> 数据来源为微软公开技术博客（2025-10 / 2025-12），属**业界公开报道**，不是本体系认可的实测基准数据，引用时请注明。
