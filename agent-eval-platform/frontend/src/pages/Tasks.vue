  ...
  <div>
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
      <div>
        ...
        <p class="page-sub">在线持续评估生产轨迹 · 离线批量评估数据集 · 状态机：创建 → 运行中 ⇄ 已暂停 → 异常 → 已终止</p>
      </div>
    ...
    <el-table v-loading="loading" :data="items" style="width:100%" @row-click="row => $router.push('/tasks/' + row.id)">
      ...
      <el-table-column label="已评估" width="100"><template #default="{ row }">{{ row.stats?.count ?? 0 }}</template></el-table-column>
      <el-table-column label="平均分" width="90"><template #default="{ row }">{{ (row.stats?.avg ?? 0).toFixed(2) }}</template></el-table-column>
...
const tab = ref('online'), items = ref([]), apps = ref([]), evs = ref([]), datasets = ref([]), loading = ref(false), dlg = ref(false), editDlg = ref(false)
const editRow = ref(null), editForm = ref({ evaluator_ids: [], threshold: 0.6 })
const form = reactive({ name: '', mode: 'online', app_id: '', dataset_id: '', evaluator_ids: [], threshold: 0.6, sample_rate: 100 })
