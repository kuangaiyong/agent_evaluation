...
>
> - 原文链接：https://mp.weixin.qq.com/s/6kpzWl4pkZJe-oJluxgZjg
> - 公众号：AliExpress技术
> - 发布日期：2026-07-21
> - 抓取日期：2026-08-11
> - 随文图片：13 张，存于 `assets/13-AI-Agent应用精细化评测-评测体系设计与工程实践/`
...
![图 1](assets/13-AI-Agent应用精细化评测-评测体系设计与工程实践/img-01.png)
...
![图 2](assets/13-AI-Agent应用精细化评测-评测体系设计与工程实践/img-02.svg)
...
![图 3](assets/13-AI-Agent应用精细化评测-评测体系设计与工程实践/img-03.svg)
...
| Task 类型 | 核心逻辑 | 适用指标 | 判断标准 | 量化方法 |
| --- | --- | --- | --- | --- |
| 二元判断 | 判断 Agent 是否在语义上完成了期望行为，适用于"是否完成/是否正确"的判断场景。 | 任务完成率 | LLM 判断 Agent 是否语义完成了核心任务 → pass=true/false → 聚合 passed / total 得通过率 | 输出字段：passed：是否通过reasoning：CoT 推理过程指标计算：通过样本数 / 总样本数 × 100% |
| 多轮对话完成率 | LLM 判断多轮对话中 Agent 是否逐轮响应 → 同上 |  |  |  |
| 异常输入处理率 | LLM 判断 Agent 是否返回了合理引导而非崩溃 → 同上 |  |  |  |
| 意图识别准确率 | LLM 判断 Agent 识别意图是否与标注意图语义一致 → 同上 |  |  |  |
| 意图识别精确率 | 不调 LLM，通过埋点判断是否命中skill → 同上 |  |  |  |
| 意图识别召回率 | 不调 LLM，通过埋点判断命中skill 是否符合预期→ 同上 |  |  |  |
| 降级触发准确率 | 不调 LLM，通过埋点判断未命中 Skill 是否降级知识库检索→ 同上 |  |  |  |
| 路由决策准确率 | 不调 LLM，通过埋点判断Agent 选择的执行路径（Skill调用 / RAG检索）是否符合预期→ 同上 |  |  |  |
| 知识检索决策准确率 | 不调 LLM，通过埋点判断是否正确触发知识库检索→ 同上 |  |  |  |
| 工具调用成功率 | 不调 LLM，通过埋点判断是否成功返回有效结果→ 同上 |  |  |  |
| 一致性判断 | 核查 Agent 输出与参考标准的一致性，并列出具体不一致项 | 幻觉率（忠实性） | LLM 判断 Agent 生成内容是否可在知识库检索结果中找到依据，无凭空捏造的信息，如有不一致项做归因分析 | 输出字段：passed：是否通过reasoning：CoT 推理过程faithful：是否忠实于知识源hallucinatedClaims：具体的幻觉内容列表指标计算：faithful为false的样本数 / 总样本数 × 100% |
| 指令遵循能力 | LLM 判断 Agent 生成内容的格式、结构、必填字段是否符合 Skill 定义的约束要求，否则做违规项分析 | 输出字段：passed：是否通过reasoning：CoT 推理过程compliant：是否遵循规范violations：具体违规项列表指标计算：compliant为true的样本数 / 总样本数 × 100% |  |  |
| 多标签匹配 | 对比期望列表与实际列表的交集/差集，支持部分匹配 | 多意图识别率 | LLM 逐一对比 Agent 识别意图与期望意图列表，判断每个期望意图是否被覆盖 | 输出字段：passed：是否通过reasoning：CoT 推理过程expected：期望的标签列表matched：已匹配的标签missed：遗漏的标签extra：多余的标签指标计算：多意图识别率 = 所有期望意图均被匹配的样本数 / 总样本数 × 100%工具调用决策/调用准确率 = matched完全覆盖expected的样本数 / 总样本数 × 100%参数映射准确率 = 所有字段均正确的样本数 / 总样本数 × 100% |
| 工具调用决策准确率工具调用准确率 | LLM 逐一比对 Agent 调用工具与期望工具列表，判断每个期望工具是否被覆盖 |  |  |  |
|  |  |  |  |  |
| 参数映射准确率 | LLM 逐一比对每个参数字段，判断 Agent 是否正确映射了用户意图到工具参数 |  |  |  |
| 上下文保留 | 评估多轮对话中 Agent 是否正确引用了历史上下文 | 短期记忆保留率 | LLM 逐轮判断 Agent 回复中是否正确引用或体现了标注的关键上下文信息 | 输出字段：passed：是否通过reasoning：CoT 推理过程turnNumber：当前轮次编号retained：是否保留了上下文expectedRefs：期望的引用foundRefs ：实际的引用missedRefs：遗漏的引用指标计算：短期记忆保留率 = retained为true的样本数 / 总样本数 × 100%记忆衰减曲线 = 按轮次分组统计：第N轮retained为true的样本数 / 第N轮总样本数 × 100% |
| 记忆衰减曲线 | LLM 判断 Agent 是否仍能正确引用早期对话中的关键信息，观察 Agent 随对话增长的记忆退化规律 |  |  |  |
| 相关性判断 | 评估 RAG 检索结果质量，输出相关/不相关片段的数量比 | 长期记忆检索精确率 | LLM 判断检索回来的内容有多少是相关的，输出相关片段数和总片段数的比值 | 输出字段：passed：是否通过reasoning：CoT 推理过程totalChunks：总返回 chunk 数 / 期望 chunk 数relevantChunks：相关 chunk 数 / 覆盖 chunk 数irrelevantChunks ：不相关 chunk 描述指标计算：长期记忆检索精确率 = 相关 chunk 数 / 总返回 chunk 数 × 100%长期记忆检索召回率 =覆盖 chunk 数 / 期望 chunk 数 × 100% |
| 长期记忆检索召回率 | LLM 判断期望的知识点有多少被覆盖了，输出期望知识点列表和已覆盖知识点的比值，并列出未覆盖的知识点便于定位检索盲区。 |  |  |  |
| 行为判断 | 适用于判断 Agent 是否执行了特定交互行为的场景 | 模糊意图澄清率 | LLM 对 Agent 行为的判断，包括主动追问、合理推断、答非所问、产生幻觉。前两种视为通过，后两种视为不通过。 | 输出字段：passed：是否通过reasoning：CoT 推理过程clarified：是否进行了澄清behavior：行为类型枚举（4 种）指标计算：clarified为true的样本数 / 总样本数 × 100% |
...
![图 4](assets/13-AI-Agent应用精细化评测-评测体系设计与工程实践/img-04.svg)
...
评测体系的最后一环是可视化。本文初步构建了一个轻量级的评测平台，只需选择评测范围即可自动装配对应的评测数据集，同时拉取该范围相关的埋点指标。评测范围一变，数据集和埋点指标自动跟着调整，无需人工逐个对接数据源。通过“选择评测范围 → 自动装配数据集 → 自动筛选对应指标”，可以使评测过程简单而灵活。
...
![图 5](assets/13-AI-Agent应用精细化评测-评测体系设计与工程实践/img-05.png)
...
![图 6](assets/13-AI-Agent应用精细化评测-评测体系设计与工程实践/img-06.png)

### **7.2 多维指标**

**评测结果**：评测完成后，下方首先展示总用例数、通过数、失败数和通过率，支持导出 JSON 评测数据。

![图 7](assets/13-AI-Agent应用精细化评测-评测体系设计与工程实践/img-07.png)
...
![图 8](assets/13-AI-Agent应用精细化评测-评测体系设计与工程实践/img-08.png)

![图 9](assets/13-AI-Agent应用精细化评测-评测体系设计与工程实践/img-09.png)

**场景通过率**：以进度条的形式展示各业务场景（Trace 排查、错误码分析、标签使用分析、可见可售性分析、领域知识问答等）的通过率，按通过率从高到低排序，快速定位薄弱环节。

![图 10](assets/13-AI-Agent应用精细化评测-评测体系设计与工程实践/img-10.png)
...
![图 11](assets/13-AI-Agent应用精细化评测-评测体系设计与工程实践/img-11.png)
...
![图 12](assets/13-AI-Agent应用精细化评测-评测体系设计与工程实践/img-12.png)
...
![图 13](assets/13-AI-Agent应用精细化评测-评测体系设计与工程实践/img-13.png)
...
| 评测指标：用户问题解决、任务相关性、逻辑一致性、信息准确性、内容可读性 | 评测指标：端到端评测：任务完成质量、任务成本消耗、任务完成效率和安全鲁棒性四个维度，共计 11 项指标核心模块评测：感知模块、规划模块、记忆模块和工具模块四个维度，共计 24 项指标 |
| 评测流程：AppBuilder 搭建 Agent应用和评估器 + AE质量评测平台 | 评测流程：自建Agent + 自动化评测 + 评测指标可视化 |
